<?php

require_once ('../db_config.php');
require_once ('../essentials.php');
adminLogin();

if (isset($_POST['add_carousel'])) {
    
    $img_r = uploadImage($_FILES['picture'], CAROUSEL_FOLDER_PATH);

    if ($img_r == 'inv_img') {
        echo $img_r;
    } else if ($img_r == 'inv_size') {
        echo $img_r;
    } else if ($img_r == 'upd_failed') {
        echo $img_r;
    } else {
        $q = "INSERT INTO `carousel`(`image`) VALUES (?)";
        $values = [$img_r];
        $res = insert($con, $q, $values, 's');
        echo $res;
    }
}

if (isset($_POST['get_carousel'])) {
    $res = selectAll($con,'carousel');
    while ($data = mysqli_fetch_assoc($res)) {
        $path = Carousel_IMG_PATH;
        echo <<<data
                <div class="col-md-4 mb-3">
                                <div class="card bg-dark text-white">
                                    <img src="$path$data[image]" class="card-img" alt="Image not found">
                                    <div class="card-img-overlay text-end">
                                        <button type="button" onclick="remove_carousel($data[sr_no])" class="btn btn-danger btn-sm">
                                            <i class="bi bi-trash"></i>Delete
                                        </button>                                        
                                    </div>                                    
                                </div>
                </div>
            data;
    }
}

if (isset($_POST['remove_carousel'])) {
    $frm_data = filtration($_POST);
    $values = [$frm_data['remove_carousel']];

    $pre_q = "SELECT * FROM `carousel` WHERE `sr_no`=?";
    $res = select($con, $pre_q, $values, 'i');
    $img = mysqli_fetch_assoc($res);

    if (deleteImage($img['image'], CAROUSEL_FOLDER_PATH)) {
        $q = "DELETE FROM `carousel` WHERE `sr_no`=?";
        $res = delete($con, $q, $values,'i');
        echo $res;
    } else {
        echo 0;
    }
}